...
class Archea(Agent):
    def __init__(
        ...
        self._obs_dim = (
            self._sensor_obs_coord + 3
        )  # +1 for is_colliding_evader, +1 for is_colliding_poison, +1 for angle
        # self._observation_space_low = np.ones(shape=(self._obs_dim,)) * -np.sqrt(2)
        # self._observation_space_low[-1] = 0
        # self._observation_space_high = np.ones(shape=(self._obs_dim,)) * 2. * np.sqrt(2)
        # self._observation_space_high[-1] = 2. * np.pi
        self._observation_space_low = np.float32(-np.sqrt(2))
        self._observation_space_high = np.float32(2.0 * np.sqrt(2))
    ...
    def observation_space(self):
        return spaces.Box(
            low=self._observation_space_low,
            high=self._observation_space_high,
            shape=(self._obs_dim,),
            dtype=np.float32,
        )
    ...
    def action_space(self):
        return spaces.Box(
            low=np.array([-self._max_accel, -self._max_accel, -self._max_angle_change]),
            high=np.array([self._max_accel, self._max_accel, self._max_angle_change]),
            shape=(3,),
            dtype=np.float32,
        )
    ...
    def add_angle(self, add_angle):
        # ensure angle is in range of 0, 2pi
        self._angles = (self._angles + add_angle) % (2 * np.pi)
        sensor_vectors = np.c_[np.cos(self._angles), np.sin(self._angles)]
        self._sensors = sensor_vectors
    ...
    def sense_angle(self):
        # convert first angle to be in range -sqrt(2) to 2sqrt(2)
        return (self._angles[0] / (2 * np.pi) * (3 * np.sqrt(2))) - np.sqrt(2)
...
class MAWaterWorld:
    def __init__(
        ...
        radius=0.015,
        obstacle_radius=0.2,
        obstacle_coord=np.array([0.5, 0.5]),
    ...
    def collision_handling_subroutine(self, rewards, is_last):
        ...
        # Find angles
        pursuer_angles = np.array([pursuer.sense_angle() for pursuer in self._pursuers])
    ...
    def observe_list(
        self, sensor_feature, is_colliding_evader, is_colliding_poison, pursuer_angles
    ):
        obslist = []
        for pursuer_idx in range(self.n_pursuers):
            obslist.append(
    ...
    def step(self, action, agent_id, is_last):
        ...
        add_angle = action[2]
        speed_action = speed_action.reshape(2)
        speed = np.linalg.norm(speed_action)
        if speed > self.pursuer_max_accel:
            # Limit added thrust to self.pursuer_max_accel
            speed_action = speed_action / speed * self.pursuer_max_accel

        p = self._pursuers[agent_id]
        p.add_angle(add_angle)
        p.set_velocity(p.velocity + speed_action)
