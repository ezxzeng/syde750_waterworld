...
class Archea(Agent):
    ...
    def __init__(self, idx, radius, n_sensors, sensor_range, max_accel, max_angle_change=1, speed_features=True):
        self._idx = idx
        self._radius = radius
    ...
    def action_space(self):
        return spaces.Box(low=np.array([-self._max_accel, -self._max_accel, -self._max_angle_change]), high=np.array([self._max_accel, self._max_accel, self._max_angle_change]), shape=(3,), dtype=np.float32)
...
class MAWaterWorld():
    ...
    def __init__(self, n_pursuers=5, n_evaders=5, n_poison=10, n_coop=2, n_sensors=30, sensor_range=0.2,
                 ...
                 speed_features=True, max_cycles=500, max_angle_change=1, **kwargs):
        """
        ...
        self._pursuers = [
            Archea(pursuer_idx + 1, self.radius, self.n_sensors, sensor_range, self.pursuer_max_accel,
                   speed_features=self._speed_features, max_angle_change=self.max_angle_change)
