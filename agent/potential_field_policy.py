...
class PotentialFieldPolicy:
    def __init__(self, n_sensors=20, speed_features=True, pursuer_max_accel=0.01):
    ...
    def get_movement_vector(self, observation):
        ...
        repulsion_distances = [obs_dist, poison_dist, barr_dist]
        attraction_distances = [food_dist]
        max_repulsion_val = np.max(np.concatenate(repulsion_distances))
        repulsion = self.get_force_sensors(repulsion_distances, replace=(1, max_repulsion_val), subtract=max_repulsion_val)
    ...
    def get_force_sensors(self, distances, replace=(1, 0), subtract=np.sqrt(2), set_val=None):
